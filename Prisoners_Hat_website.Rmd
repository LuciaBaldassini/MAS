...
Next the announcement are made. Although this part of the system has still do be finished, we think of having a loop called ```announcementLoop()``` in which, after each announcement, the knowledge of each agent is updated with a function ```updateKnowledge()```
